...
class adjective():
    ...
    def file_read(self):
        seed_file = open(self.filename, 'r')
        csv_read = csv.reader(seed_file, delimiter=' ')
        for row in csv_read:
...
adjectiv = ['beautiful', 'concrete', 'quick', 'worse',
            'cheap', 'good', 'nice', 'excellent', 'slow']
cl = adjective(filename, adjectiv)
cl.file_read()
[pos, neg, new_list] = cl.orientation()
cl.file_write()
print(pos, neg, new_list)
'''
